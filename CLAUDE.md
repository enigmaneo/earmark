...
Schema is managed with **Alembic** (`migrations/`, config in `alembic.ini`). The app runs
`alembic upgrade head` automatically at startup (`init_db` in `src/earmark/database.py`), so a
fresh database is created and existing ones are upgraded on boot. A pre-Alembic database (app
tables but no `alembic_version`) is auto-adopted: it's stamped at the baseline revision and then
upgraded.

```bash
# after changing a model in src/earmark/models.py:
uv run alembic revision --autogenerate -m "describe change"   # generate a migration
uv run alembic upgrade head                                   # apply migrations
uv run alembic downgrade -1                                   # roll back one
uv run python -m earmark.reset_db                             # drop everything + re-migrate (dev only)
```

> SQLite can't `ALTER` most constraints, so `env.py` enables Alembic batch mode
> (`render_as_batch=True`). Review autogenerated migrations and **name any constraints**
> (batch mode rejects unnamed ones).
